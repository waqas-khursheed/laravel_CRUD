<table id="datatable_student_feetype" class="table table-striped table-bordered table-hover dataTable no-footer" width="100%">
    ...
    <thead>
        <tr>
            ...
            <th class="hasinput" >
                <input type="text" class="form-control" placeholder="{{ __('a_data.filter')}} {{ __('a_data.delete')}}" />
            </th>
        ...
        <tr>
            ...
            <th width="5%" class="text-center">{{ __('a_data.delete') }}</th>
        </tr>
    </thead>
    <tbody>
        @foreach ($user['page_data'] as $data)
            <tr>
                ...
                <td>{{ $data->code }}</td>
                <td align="center">
                    <input class='tgl tgl-skewed tgl-skewed-xedit_fee_type_askD' id='record_deleted-{{$data->id}}' type='checkbox' <?php echo ($data->record_deleted == 1 ? 'checked' : ''); ?> />
